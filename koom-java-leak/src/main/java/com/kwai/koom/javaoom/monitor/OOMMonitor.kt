...
object OOMMonitor : LoopMonitor<OOMMonitorConfig>(), LifecycleEventObserver {
    ...
    private fun processOldHprofFile() {
        ...
        if (mHasProcessOldHprof) {
            MonitorLog.i(TAG, "processHprofFile mHasProcessOldHprof return;")
            return
        }
        mHasProcessOldHprof = true
        MonitorLog.i(TAG, "reAnalysisHprof")
        reAnalysisHprof()
        MonitorLog.i(TAG, "manualDumpHprof")
        manualDumpHprof()
    }

    private fun reAnalysisHprof() {
        // 分析现有的hprof文件
        MonitorLog.i(TAG, "hprofAnalysisDir.listFiles() = " + hprofAnalysisDir.absolutePath)
        for (file in hprofAnalysisDir.listFiles().orEmpty()) {
            if (!file.exists()) continue // 文件不存在

            if (!file.name.startsWith(MonitorBuildConfig.VERSION_NAME)) { // "1.0.0" , 检查版本号
                MonitorLog.i(TAG, "delete other version files ${file.name}")
                file.delete()
                continue
            }

            if (file.canonicalPath.endsWith(".hprof")) { // 后缀是hprof的文件
                val jsonFile = File(file.canonicalPath.replace(".hprof", ".json")) // 同名文件，不同后缀
                if (!jsonFile.exists()) { // 文件不存在,创建文件,并hprof->json
