...
internal class HprofInMemoryIndex private constructor(
    ...
    fun classId(className: String): Long? {
        ...
        val hprofStringId: Long? = hprofStringCache.entrySequence().firstOrNull {
            it.second == internalClassName
        }?.first
    ...
    fun objectAtIndex(index: Int): LongObjectPair<IndexedObject> {
        ...
        if (index < classIndex.size) {
            val objectId: Long = classIndex.keyAt(index)
            val array: ByteSubArray = classIndex.getAtIndex(index)
    ...
    override fun toString(): String {
        if(true) return "";
        HprofHeapGraph.log("HprofInMemoryIndex HprofInMemoryIndex(positionSize=$positionSize, \n")

        HprofHeapGraph.log("HprofInMemoryIndex hprofStringCache=$hprofStringCache, \n")
//        for (longObjectPair: LongObjectPair<String> in hprofStringCache.entrySequence()) {
//            HprofHeapGraph.log(
//                "hprofStringCache first->${longObjectPair.first} , second->${
//                    longObjectPair.second
//                }"
//            )
//        }

        HprofHeapGraph.log("HprofInMemoryIndex classNames=$classNames, \n")
        classNames.forEach(object : LongLongScatterMap.ForEachCallback {
            override fun onEntry(key: Long, value: Long) {
                HprofHeapGraph.log("classNames key=$key, value=$value")
            }
        })


        HprofHeapGraph.log("HprofInMemoryIndex classIndex=$classIndex, \n")
        for (pair in classIndex.entrySequence()) {
            HprofHeapGraph.log("classIndex first->${pair.first} second->${pair.second}")
        }

        HprofHeapGraph.log("HprofInMemoryIndex instanceIndex=$instanceIndex, \n")
        HprofHeapGraph.log("HprofInMemoryIndex objectArrayIndex=$objectArrayIndex, \n")
        HprofHeapGraph.log("HprofInMemoryIndex primitiveArrayIndex=$primitiveArrayIndex, \n")
        HprofHeapGraph.log("HprofInMemoryIndex gcRoots=$gcRoots, \n")
        HprofHeapGraph.log("HprofInMemoryIndex proguardMapping=$proguardMapping, \n")
        HprofHeapGraph.log("HprofInMemoryIndex bytesForClassSize=$bytesForClassSize, \n")
        HprofHeapGraph.log("HprofInMemoryIndex bytesForInstanceSize=$bytesForInstanceSize, \n")
        HprofHeapGraph.log("HprofInMemoryIndex bytesForObjectArraySize=$bytesForObjectArraySize, \n")
        HprofHeapGraph.log("HprofInMemoryIndex bytesForPrimitiveArraySize=$bytesForPrimitiveArraySize, \n")
        HprofHeapGraph.log(
            "HprofInMemoryIndex " +
                "useForwardSlashClassPackageSeparator=$useForwardSlashClassPackageSeparator,"
        )
        HprofHeapGraph.log("HprofInMemoryIndex classFieldsReader=$classFieldsReader, \n")
        HprofHeapGraph.log("HprofInMemoryIndex classFieldsIndexSize=$classFieldsIndexSize, \n")
        HprofHeapGraph.log("HprofInMemoryIndex classCount=$classCount, \n")
        HprofHeapGraph.log("HprofInMemoryIndex instanceCount=$instanceCount, \n")
        HprofHeapGraph.log("HprofInMemoryIndex objectArrayCount=$objectArrayCount, \n")
        HprofHeapGraph.log("HprofInMemoryIndex primitiveArrayCount=$primitiveArrayCount)")
        return ""
    }
    ...
    private class Builder(
         ...
         */
        private val classNames: LongLongScatterMap = LongLongScatterMap(expectedElements = classCount)

        private val classFieldBytes: ByteArray = ByteArray(classFieldsTotalBytes)

        private var classFieldsIndex: Int = 0
        ...
        private val gcRoots: MutableList<GcRoot> = mutableListOf<GcRoot>()
    ...
    companion object {
        ...
        fun indexHprof(
            ...
            SharkLog.d {
                "classCount:${classCount} instanceCount:${instanceCount} " +
