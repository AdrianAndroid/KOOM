...
sealed class HeapObject {
    ...
    class HeapClass internal constructor(
         ...
         */
        val isPrimitiveWrapperClass: Boolean get() = (name in primitiveWrapperClassNames)
        ...
        val isPrimitiveArrayClass: Boolean get() = name in primitiveTypesByPrimitiveArrayClassName

        val isObjectArrayClass: Boolean get() = isArrayClass && !isPrimitiveArrayClass
         ...
         */
        val directInstances: Sequence<HeapInstance> get() = hprofGraph.instances.filter { it.indexedObject.classId == objectId }
        ...
        private fun readRecordStaticFields() = hprofGraph.classDumpStaticFields(indexedObject)
        ...
        fun readStaticFields(): Sequence<HeapField> {
            return readRecordStaticFields().asSequence()
                    .map { fieldRecord ->
                        HeapField(this, hprofGraph.staticFieldName(objectId, fieldRecord), HeapValue(hprofGraph, fieldRecord.value))
                    }
        }
        ...
        override fun toString(): String {
            if (true) return ""
            HprofHeapGraph.log("HeapClass ========start=========")
            HprofHeapGraph.log("HeapClass hprofGraph=$hprofGraph, ")
            HprofHeapGraph.log("HeapClass indexedObject=$indexedObject, ")
            HprofHeapGraph.log("HeapClass objectId=$objectId, ")
            HprofHeapGraph.log("HeapClass objectIndex=$objectIndex, ")
            HprofHeapGraph.log("HeapClass graph=$graph, ")
            HprofHeapGraph.log("HeapClass isPrimitiveWrapperClass=$isPrimitiveWrapperClass, ")
            HprofHeapGraph.log("HeapClass name='$name', ")
            HprofHeapGraph.log("HeapClass simpleName='$simpleName',")
            HprofHeapGraph.log("HeapClass  instanceByteSize=$instanceByteSize, ")
            HprofHeapGraph.log("HeapClass recordSize=$recordSize, ")
            HprofHeapGraph.log("HeapClass hasReferenceInstanceFields=$hasReferenceInstanceFields,")
            HprofHeapGraph.log("HeapClass isArrayClass=$isArrayClass, ")
            HprofHeapGraph.log("HeapClass isPrimitiveArrayClass=$isPrimitiveArrayClass, ")
            HprofHeapGraph.log("HeapClass isObjectArrayClass=$isObjectArrayClass, ")
            HprofHeapGraph.log("HeapClass superclass=$superclass,")
            HprofHeapGraph.log("HeapClass _classHierarchy=$_classHierarchy, ")
            HprofHeapGraph.log("HeapClass classHierarchy=$classHierarchy, ")
            HprofHeapGraph.log("HeapClass subclasses=$subclasses, ")
            HprofHeapGraph.log("HeapClass instances=$instances, ")
            HprofHeapGraph.log("HeapClass objectArrayInstances=$objectArrayInstances, ")
            HprofHeapGraph.log("HeapClass primitiveArrayInstances=$primitiveArrayInstances, ")
            HprofHeapGraph.log("HeapClass directInstances=$directInstances))")
            HprofHeapGraph.log("HeapClass ========end=========")
            return ""
        }
    ...
    class HeapInstance internal constructor(
        ...
        fun readField(declaringClassName: String, fieldName: String): HeapField? {
            return readFields().firstOrNull { field: HeapField -> field.declaringClass.name == declaringClassName && field.name == fieldName }
        }
         ...
         * @see readField
         */
        operator fun get(declaringClassName: String, fieldName: String) = readField(declaringClassName, fieldName)
        ...
        fun readFields(): Sequence<HeapField> {
            val fieldReader: FieldValuesReader by lazy {
    ...
    class HeapObjectArray internal constructor(
         ...
         */
        val arrayClassSimpleName: String get() = classSimpleName(arrayClassName)

        /**
         * The class of this array.
         */
        val arrayClass: HeapClass get() = hprofGraph.findObjectById(indexedObject.arrayClassId) as HeapClass
    ...
    class HeapPrimitiveArray internal constructor(
         ...
         */
        val primitiveType: PrimitiveType get() = indexedObject.primitiveType

        /**
         * The name of the class of this array, identical to [Class.getName].
         */
        val arrayClassName: String get() = "${primitiveType.name.toLowerCase(Locale.US)}[]"

        /**
         * The class of this array.
         */
        val arrayClass: HeapClass get() = graph.findClassByName(arrayClassName)!!
