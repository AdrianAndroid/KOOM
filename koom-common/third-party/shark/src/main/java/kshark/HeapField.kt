...
class HeapField(
     ...
     */
    val valueAsClass: HeapClass? get() = value.asObject?.asClass

    /**
     * Return a [HeapInstance] is [value] references an instance, and null otherwise.
     */
    val valueAsInstance: HeapInstance? get() = value.asObject?.asInstance

    /**
     * Return a [HeapObjectArray] is [value] references an object array, and null otherwise.
     */
    val valueAsObjectArray: HeapObjectArray? get() = value.asObject?.asObjectArray

    /**
     * Return a [HeapPrimitiveArray] is [value] references a primitive array, and null
     * otherwise.
     */
    val valueAsPrimitiveArray: HeapPrimitiveArray? get() = value.asObject?.asPrimitiveArray
}