...
data class HprofHeader(
    ...
    val heapDumpTimestamp: Long = System.currentTimeMillis(), // u8
    /** Hprof version, which is tied to the runtime where the heap was dumped. */
    val version: HprofVersion = HprofVersion.ANDROID, // [u1]*
    /**
     ...
     * required to be.
     */
    val identifierByteSize: Int = 4 // u4
) {
    /**
    ...
    companion object {
        private val supportedVersions: Map<String, HprofVersion> = HprofVersion.values()
        ...
        fun parseHeaderOf(source: BufferedSource): HprofHeader {
            ...
            val version: HprofVersion? = supportedVersions[versionName]
            // 中间的检查
            ...
            source.skip(1)
            val identifierByteSize: Int = source.readInt()
            val heapDumpTimestamp: Long = source.readLong()
