...
class HprofHeapGraph internal constructor(
    ...
    override fun findClassByName(className: String): HeapClass? {
        val heapDumpClassName: String = if (header.version != ANDROID) {
...
//    }
    ...
    companion object {
        ...
        fun log(msg: String) {
            SharkLog.logger?.d("HprofHeapGraph : $msg");
        }
        ...
        fun File.openHeapGraph(
            ...
            return FileSourceProvider(file = this).openHeapGraph(proguardMapping, indexedGcRootTypes)
        }
        ...
        fun DualSourceProvider.openHeapGraph(
            ...
            log(header.toString())
            val index: HprofIndex = HprofIndex.indexRecordsOf(
                hprofSourceProvider = this,
                hprofHeader = header, // header:指定的头格式
                proguardMapping = proguardMapping, // 此时传空，先不考虑
                indexedGcRootTags = indexedGcRootTypes // 要收集的集合的tag's
            )
