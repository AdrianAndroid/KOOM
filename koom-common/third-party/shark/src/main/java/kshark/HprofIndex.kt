...
class HprofIndex private constructor(
    ...
    fun openHeapGraph(): CloseableHeapGraph {
        val reader: RandomAccessHprofReader = RandomAccessHprofReader.openReaderFor(sourceProvider, header)
        return HprofHeapGraph(header, reader, index)
    }

    override fun toString(): String {
        return "HprofIndex(sourceProvider=$sourceProvider, header=$header, index=$index)"
    }
    ...
    companion object {
        ...
        fun indexRecordsOf(
            ...
            val reader: StreamingHprofReader = StreamingHprofReader.readerFor(hprofSourceProvider, hprofHeader)
            val index: HprofInMemoryIndex = HprofInMemoryIndex.indexHprof(
                reader = reader,
                hprofHeader = hprofHeader,
