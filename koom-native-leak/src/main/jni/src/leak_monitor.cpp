...
// can ignore in LP64 So we can't proxy aligned_alloc/valloc/pvalloc.
        ...
        bool LeakMonitor::Install(std::vector<std::string> *selected_list, std::vector<std::string> *ignore_list) {
            ...
            std::vector<const std::string> ignore_pattern = {".*/libkoom-native.so$", ".*/libxhook_lib.so$"};
